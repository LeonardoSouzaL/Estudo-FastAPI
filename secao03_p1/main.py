...
@app.get('/cursos/{curso_id}', 
         description='Retorna o curso desejado pelo ID ou Não Existe', 
         summary='Retorna o curso desejado por ID')
...
@app.post('/cursos', status_code=status.HTTP_201_CREATED, 
         description='Cria um novo curso ou responde caso já exista o mesmo.', 
         summary='Criar Curso',
...
async def put_curso(curso_id: int, curso: Curso, db: Any = Depends(fake_db)):
    if curso_id in cursos:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                             detail=f'Não existe um curso com esse id {curso_id}.')
